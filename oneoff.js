...
const {default: redisconn} = require('./redisconn');
...
(async () => {
    ...
    for (let k = fromEra; k <= targetEra; k++) {
        ...
        await jsonCache.set(`${suffix}:validators:top:${k}`, topValidators);
        await jsonCache.set(`${suffix}:validators:bottom:${k}`, bottomValidators);

        await jsonCache.set(`${suffix}:nominators:top:${k}`, topNominators);
        await jsonCache.set(`${suffix}:nominators:bottom:${k}`, bottomNominators);
