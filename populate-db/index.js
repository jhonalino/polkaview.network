...
(async () => {
    ...
    if (args.length > 2 && args[2] === 'kusama') { // if there is a command line arg for kusama, use kusama network
        ...
        network = 'kusama'
        console.log(networks.kusama.wshost, 'hehe');
        provider = new WsProvider(networks.kusama.wshost);
        DOT_DECIMAL_PLACES *= 100
    }
    else { // default to polkadot
        console.log('Connecting to Polkadot')
        console.log('olaw', networks);
        console.log(networks.polkadot.wshost);
        //provider = new WsProvider('wss://rpc.polkadot.io')
        provider = new WsProvider(networks.polkadot.wshost);
    }
